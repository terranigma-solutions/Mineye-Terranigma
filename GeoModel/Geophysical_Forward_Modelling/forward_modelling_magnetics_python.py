...
def create_dipole_sphere_model(sphere_center=(0, 0, -500), sphere_radius=200, 
                              susceptibility=0.1, background_susceptibility=0.001,
                              model_extent=(-2000, 2000, -2000, 2000, -1000, 100),
                              model_resolution=(20, 20, 15)):
    """
    Create a spherical dipole model for testing magnetic forward modeling.
    
    This creates a high-susceptibility sphere in a lower-susceptibility background,
    which approximates a magnetic dipole source.
    
    Args:
        sphere_center: (x, y, z) center of the sphere in meters
        sphere_radius: Radius of the sphere in meters
        susceptibility: Magnetic susceptibility of the sphere (SI units)
        background_susceptibility: Background susceptibility (SI units)
        model_extent: (xmin, xmax, ymin, ymax, zmin, zmax) in meters
        model_resolution: (nx, ny, nz) number of voxels in each direction
        
    Returns:
        tuple: (susceptibility_3d, voxel_centers, voxel_sizes, measurement_points)
    """
    print(f"Creating spherical dipole model...")
    print(f"  Sphere center: {sphere_center}")
    print(f"  Sphere radius: {sphere_radius} m")
    print(f"  Sphere susceptibility: {susceptibility}")
    print(f"  Background susceptibility: {background_susceptibility}")
    
    nx, ny, nz = model_resolution
    x_min, x_max, y_min, y_max, z_min, z_max = model_extent
    ...
    susceptibility_3d = np.full((nx, ny, nz), background_susceptibility)
    ...
    # Set high susceptibility inside sphere
    sphere_mask = distances <= sphere_radius
    susceptibility_3d[sphere_mask] = susceptibility
    
    # Create measurement grid (survey at surface)
    survey_x = np.linspace(x_min + 400, x_max - 400, 20)
    survey_y = np.linspace(y_min + 400, y_max - 400, 20)
    survey_z = z_max + 200  # 200m above surface
    
    SX, SY = np.meshgrid(survey_x, survey_y)
    measurement_points = np.column_stack([SX.ravel(), SY.ravel(), 
                                        np.full(SX.size, survey_z)])
    
    print(f"  Model grid: {nx} x {ny} x {nz} = {nx*ny*nz} voxels")
    print(f"  Voxels in sphere: {np.sum(sphere_mask)}")
    print(f"  Measurement points: {len(measurement_points)}")
    
    return susceptibility_3d, voxel_centers, voxel_sizes, measurement_points


def create_vertical_dyke_model(dyke_center=(0, 0), dyke_width=200, dyke_thickness=100,
                              susceptibility=0.05, background_susceptibility=0.001,
                              model_extent=(-2000, 2000, -2000, 2000, -1000, 100),
                              model_resolution=(25, 25, 15)):
    """
    Create a vertical dyke model for testing magnetic forward modeling.
    
    This creates a thin vertical dyke with high susceptibility, which produces
    a characteristic magnetic anomaly pattern.
    
    Args:
        dyke_center: (x, y) center of the dyke in meters
        dyke_width: Width of the dyke (E-W direction) in meters
        dyke_thickness: Thickness of the dyke (N-S direction) in meters
        susceptibility: Magnetic susceptibility of the dyke (SI units)
        background_susceptibility: Background susceptibility (SI units)
        model_extent: (xmin, xmax, ymin, ymax, zmin, zmax) in meters
        model_resolution: (nx, ny, nz) number of voxels in each direction
    ...
    Returns:
        tuple: (susceptibility_3d, voxel_centers, voxel_sizes, measurement_points)
    """
    print(f"Creating vertical dyke model...")
    print(f"  Dyke center: {dyke_center}")
    print(f"  Dyke dimensions: {dyke_width} x {dyke_thickness} m")
    print(f"  Dyke susceptibility: {susceptibility}")
    print(f"  Background susceptibility: {background_susceptibility}")
    ...
    # Define dyke boundaries
    x_dyke_min = dyke_center[0] - dyke_width/2
    x_dyke_max = dyke_center[0] + dyke_width/2
    y_dyke_min = dyke_center[1] - dyke_thickness/2
    y_dyke_max = dyke_center[1] + dyke_thickness/2
    
    # Set high susceptibility inside dyke (entire vertical extent)
    dyke_mask = ((X >= x_dyke_min) & (X <= x_dyke_max) & 
                 (Y >= y_dyke_min) & (Y <= y_dyke_max))
    susceptibility_3d[dyke_mask] = susceptibility
    
    # Create measurement grid (survey at surface)
    survey_x = np.linspace(x_min + 200, x_max - 200, 25)
    survey_y = np.linspace(y_min + 200, y_max - 200, 25)
    ...
    print(f"  Model grid: {nx} x {ny} x {nz} = {nx*ny*nz} voxels")
    print(f"  Voxels in dyke: {np.sum(dyke_mask)}")
    print(f"  Measurement points: {len(measurement_points)}")
    
    return susceptibility_3d, voxel_centers, voxel_sizes, measurement_points


def create_multiple_dipole_model(dipole_positions=[(-500, 0, -300), (500, 0, -300)],
                                dipole_radii=[150, 200],
                                dipole_susceptibilities=[0.08, 0.12],
                                background_susceptibility=0.001,
                                model_extent=(-2000, 2000, -2000, 2000, -800, 200),
                                model_resolution=(30, 30, 20)):
    """
    Create a model with multiple dipole sources for testing interference patterns.
    
    Args:
        dipole_positions: List of (x, y, z) positions for each dipole
        dipole_radii: List of radii for each dipole
        dipole_susceptibilities: List of susceptibilities for each dipole
        background_susceptibility: Background susceptibility (SI units)
        model_extent: (xmin, xmax, ymin, ymax, zmin, zmax) in meters
        model_resolution: (nx, ny, nz) number of voxels in each direction
    ...
    Returns:
        tuple: (susceptibility_3d, voxel_centers, voxel_sizes, measurement_points)
    """
    print(f"Creating multiple dipole model...")
    print(f"  Number of dipoles: {len(dipole_positions)}")
    ...
    # Create susceptibility model with background
    susceptibility_3d = np.full((nx, ny, nz), background_susceptibility)
    
    # Add each dipole
    for i, (pos, radius, suscept) in enumerate(zip(dipole_positions, dipole_radii, dipole_susceptibilities)):
        print(f"  Dipole {i+1}: center={pos}, radius={radius}m, susceptibility={suscept}")
        
        # Calculate distances from dipole center
        distances = np.sqrt((X - pos[0])**2 + (Y - pos[1])**2 + (Z - pos[2])**2)
        
        # Set high susceptibility inside sphere
        dipole_mask = distances <= radius
        susceptibility_3d[dipole_mask] = suscept
    
    # Create measurement grid (survey at surface)
    survey_x = np.linspace(x_min + 300, x_max - 300, 30)
    survey_y = np.linspace(y_min + 300, y_max - 300, 30)
    survey_z = z_max + 150  # 150m above surface
...
def analytical_magnetic_dipole(measurement_points, dipole_position, dipole_moment,
                              incl=60.0, decl=0.0):
    """
    Calculate analytical magnetic anomaly for a point dipole.
    
    This provides a reference solution to validate the numerical forward modeling.
    Based on Blakely (1995) "Potential Theory in Gravity and Magnetic Applications"
    
    Args:
        measurement_points: Array of measurement locations [n_measurements, 3]
        dipole_position: (x, y, z) position of the dipole in meters
        dipole_moment: Magnetic moment of the dipole [Am²]
        incl: Magnetic inclination in degrees
        decl: Magnetic declination in degrees
        
    Returns:
        dT: Total magnetic field anomaly [nT]
    """
    print(f"Computing analytical dipole solution...")
    print(f"  Dipole position: {dipole_position}")
    print(f"  Dipole moment: {dipole_moment:.2e} Am²")
    print(f"  Magnetic field: I={incl}°, D={decl}°")
    ...
    mx = np.cos(incl_rad) * np.cos(decl_rad)
    my = np.cos(incl_rad) * np.sin(decl_rad)
    mz = np.sin(incl_rad)
    
    # Displacement vectors from dipole to measurement points
    dx = measurement_points[:, 0] - dipole_position[0]
    dy = measurement_points[:, 1] - dipole_position[1]
    dz = measurement_points[:, 2] - dipole_position[2]
    ...
    # Dipole moment components (aligned with magnetic field)
    dipole_mx = dipole_moment * mx
    dipole_my = dipole_moment * my
    dipole_mz = dipole_moment * mz
    
    # Magnetic field components (Tesla)
    # B = (μ₀/4π) * (1/r³) * [3(m·r̂)r̂ - m]
    mu0_4pi = 1e-7  # μ₀/(4π) in H/m
    
    # Dot product m·r̂
    m_dot_r = dipole_mx * ux + dipole_my * uy + dipole_mz * uz
    
    # Magnetic field components
    Bx = mu0_4pi * (1/r**3) * (3 * m_dot_r * ux - dipole_mx)
    By = mu0_4pi * (1/r**3) * (3 * m_dot_r * uy - dipole_my)
    Bz = mu0_4pi * (1/r**3) * (3 * m_dot_r * uz - dipole_mz)
    
    # Total magnetic field anomaly (projection onto Earth's field direction)
    dT = (Bx * mx + By * my + Bz * mz) * 1e9  # Convert to nT
