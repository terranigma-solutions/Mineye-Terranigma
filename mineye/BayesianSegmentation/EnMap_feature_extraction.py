...
# ============================================================
# 11. Workflow utilities: Plotting, wrapping, and saving
# ============================================================
...
def plot_feature_histograms(
    ...
    depth_keys = sorted([k for k in features.keys() if k.startswith("Depth_")])
    if not depth_keys:
        return
    ...
    for k in depth_keys:
        ...
        plt.hist(data, bins=250, color="skyblue", edgecolor="black", alpha=0.7)
        plt.title(f"Histogram of {k}\nMin: {dmin:.4f}, Max: {dmax:.4f}\nP90: {p90:.4f}, P95: {p95:.4f}, P99: {p99:.4f}")
        plt.xlabel("Value")
        plt.ylabel("Frequency")
        plt.axvline(dmin, color='red', linestyle='dashed', linewidth=1, label=f'Min: {dmin:.4f}')
        plt.axvline(dmax, color='red', linestyle='dashed', linewidth=1, label=f'Max: {dmax:.4f}')
        plt.axvline(p90, color='orange', linestyle='dotted', linewidth=1.5, label=f'P90: {p90:.4f}')
        plt.axvline(p95, color='green', linestyle='dotted', linewidth=1.5, label=f'P95: {p95:.4f}')
        plt.axvline(p99, color='purple', linestyle='dotted', linewidth=1.5, label=f'P99: {p99:.4f}')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()

        out_path = f"{out_prefix}_{k}_histogram.png"
        plt.savefig(out_path, dpi=200)
        plt.close()
        print(f"[EnMap] Saved histogram for {k} to {out_path}")


def features_to_memory_datasets(features: Dict[str, np.ndarray], meta: dict):
...
def write_features_to_files(features: Dict[str, np.ndarray], meta: dict, out_prefix: str = None,
...
def run_enmap_segmentation_workflow(
        enmap_folder: str,
        n_classes: int = 8,
        iterations: int = 400,
    ...
    """Run the full EnMap lithological segmentation workflow.

    This high-level function handles:
    1. Feature extraction from EnMap spectral cube.
    2. QA plotting (quicklooks, optional histograms, detrending diagnostic).
    3. Preparing in-memory and on-disk rasters for the segmentation workflow.
    4. Running the Bayesian segmentation.
    ...
    try:
        ...
        if plot_histograms:
            plot_feature_histograms(features, output_prefix_abs)
            print(f"[Workflow] Saved depth histograms to {output_prefix_abs}_Depth_*_histogram.png")
    except Exception as e:
        print(f"[WARN] Could not create feature quicklooks/histograms: {e}")
...
# ============================================================
# 13. Visualization utility: save per-band plots
# ============================================================
