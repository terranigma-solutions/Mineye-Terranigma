#!/usr/bin/env python3
"""
Performance comparison script for BaySeg vs BaySeg_Deprecated algorithms.
This script compares performance and memory footprint for datasets of different sizes
...
# Import placeholders for BaySegOriginal compatibility
# These would normally come from bayseg package but are not needed for performance testing
try:
    from bayseg.colors import cmap, cmap_norm
    from bayseg.ie import *
except ImportError:
    # Define minimal placeholders for missing functions
    cmap = plt.cm.viridis
    cmap_norm = None
    def compute_ie(data):
        return np.zeros(data.shape[0])
    def compute_labels_prob(data):
        return np.ones((data.shape[0], data.shape[1])) / data.shape[1]
...
def measure_performance(algorithm_func, data, n_classes, n_iterations, **kwargs) -> Dict:
    """Measure performance metrics for a given algorithm."""
    ...
    try:
        ...
        seg.fit(n=n_iterations, **fit_kwargs)
        ...
        memory_usage = max(final_memory - initial_memory, peak_memory_mb)
...
def run_performance_comparison():
    ...
    print("Starting BaySeg Performance Comparison")
    print("=" * 50)
    
    # Tharsis data configuration
    bands = {
    ...
    # ROI configurations with doubling areas for performance scaling analysis
    # Each ROI has approximately double the area of the previous one
    # Starting from ~25 km² and doubling: 25, 50, 100, 200, 400 km²
    rois = [
        ("ROI1_25km", (207500, 4172500, 212500, 4177500)),      # ~25 km² (5km x 5km)
        ("ROI2_50km", (262929, 4212429, 270071, 4219571)),      # ~50 km² (~7.1km x 7.1km)
        ("ROI3_100km", (300000, 4185000, 310000, 4195000)),     # ~100 km² (10km x 10km)
        ("ROI4_200km", (250858, 4177929, 264142, 4191071)),     # ~200 km² (~14.1km x 14.1km)
        ("ROI5_400km", (315000, 4215000, 335000, 4235000)),     # ~400 km² (20km x 20km)
    ]
    
    # Algorithm configurations
    n_classes = 6
    beta_init = 30.0
    n_iterations_small = 50  # Reduced for testing
    ...
    print(f"\nTesting {len(rois)} ROIs with {n_iterations_small} iterations each...")
    
    for roi_name, bounds in rois:
        print(f"\n--- Testing {roi_name} ---")
        
        # Get ROI information
        roi_info = get_roi_info(bounds)
        print(f"Area: {roi_info['area_km2']:.2f} km²")
        print(f"Approximate pixels: {roi_info['approx_pixels']:,}")
        
        results['roi_names'].append(roi_name)
        results['roi_areas'].append(roi_info['area_km2'])
        results['roi_pixels'].append(roi_info['approx_pixels'])
        
        try:
            # Load and preprocess data
            print("Loading data...")
            data = load_and_preprocess_data(bands, bounds, use_soil_mask=True)
            print(f"Data shape: {data.shape}")
            
            # Test current BaySeg (optimized)
            print("Testing current BaySeg (optimized)...")
            bayseg_results = measure_performance(
                BaySeg, data, n_classes, n_iterations_small,
                beta_init=beta_init, beta_jump_length=beta_jump_length,
                max_history=50, store_diagnostics=False  # Memory optimizations
            )
            
            results['bayseg_times'].append(bayseg_results['execution_time'])
            results['bayseg_memory'].append(bayseg_results['memory_usage_mb'])
            results['bayseg_success'].append(bayseg_results['success'])
            
            if bayseg_results['success']:
                print(f"  Time: {bayseg_results['execution_time']:.2f}s")
                print(f"  Memory: {bayseg_results['memory_usage_mb']:.2f} MB")
            else:
                print(f"  FAILED: {bayseg_results['error']}")
            
            # Test original BaySeg (non-optimized)
            print("Testing original BaySeg (non-optimized)...")
            deprecated_results = measure_performance(
                BaySegOriginal, data, n_classes, n_iterations_small,
                beta_init=beta_init, beta_jump_length=beta_jump_length
            )
            
            results['deprecated_times'].append(deprecated_results['execution_time'])
            results['deprecated_memory'].append(deprecated_results['memory_usage_mb'])
            results['deprecated_success'].append(deprecated_results['success'])
            
            if deprecated_results['success']:
                print(f"  Time: {deprecated_results['execution_time']:.2f}s")
                print(f"  Memory: {deprecated_results['memory_usage_mb']:.2f} MB")
            else:
                print(f"  FAILED: {deprecated_results['error']}")
            
        except Exception as e:
            print(f"  ERROR loading data: {e}")
            results['bayseg_times'].append(0)
            results['bayseg_memory'].append(0)
            results['bayseg_success'].append(False)
            results['deprecated_times'].append(0)
            results['deprecated_memory'].append(0)
            results['deprecated_success'].append(False)
    
    return results


def create_comparison_plots(results: Dict):
    """Create comparison plots for performance and memory usage."""
    
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('BaySeg vs BaySeg_Deprecated Performance Comparison', fontsize=16)
    
    roi_names = results['roi_names']
    areas = results['roi_areas']
    pixels = results['roi_pixels']
    
    # Plot 1: Execution Time vs Dataset Size (Area)
    ax1.plot(areas, results['bayseg_times'], 'bo-', label='BaySeg (Optimized)', linewidth=2, markersize=8)
    ax1.plot(areas, results['deprecated_times'], 'ro-', label='BaySeg Deprecated', linewidth=2, markersize=8)
    ax1.set_xlabel('Dataset Area (km²)')
    ax1.set_ylabel('Execution Time (seconds)')
    ax1.set_title('Execution Time vs Dataset Area')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Add ROI labels
    for i, name in enumerate(roi_names):
        if results['bayseg_success'][i]:
            ax1.annotate(name, (areas[i], results['bayseg_times'][i]), 
                        xytext=(5, 5), textcoords='offset points', fontsize=8)
    
    # Plot 2: Memory Usage vs Dataset Size (Area)
    ax2.plot(areas, results['bayseg_memory'], 'bo-', label='BaySeg (Optimized)', linewidth=2, markersize=8)
    ax2.plot(areas, results['deprecated_memory'], 'ro-', label='BaySeg Deprecated', linewidth=2, markersize=8)
    ax2.set_xlabel('Dataset Area (km²)')
    ax2.set_ylabel('Memory Usage (MB)')
    ax2.set_title('Memory Usage vs Dataset Area')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    # Plot 3: Execution Time vs Pixel Count
    ax3.plot(pixels, results['bayseg_times'], 'bo-', label='BaySeg (Optimized)', linewidth=2, markersize=8)
    ax3.plot(pixels, results['deprecated_times'], 'ro-', label='BaySeg Deprecated', linewidth=2, markersize=8)
    ax3.set_xlabel('Approximate Pixel Count')
    ax3.set_ylabel('Execution Time (seconds)')
    ax3.set_title('Execution Time vs Pixel Count')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    ax3.ticklabel_format(style='scientific', axis='x', scilimits=(0,0))
    
    # Plot 4: Performance Ratio (Speedup and Memory Efficiency)
    speedup_ratios = []
    memory_ratios = []
    
    for i in range(len(results['roi_names'])):
        if results['bayseg_success'][i] and results['deprecated_success'][i]:
            if results['bayseg_times'][i] > 0:
                speedup = results['deprecated_times'][i] / results['bayseg_times'][i]
                speedup_ratios.append(speedup)
            else:
                speedup_ratios.append(0)
                
            if results['bayseg_memory'][i] > 0:
                mem_ratio = results['deprecated_memory'][i] / results['bayseg_memory'][i]
    ...
    valid_areas = [areas[i] for i in range(len(areas)) if speedup_ratios[i] > 0]
    valid_speedups = [s for s in speedup_ratios if s > 0]
    valid_mem_ratios = [m for m in memory_ratios if m > 0]
    
    if valid_speedups:
        ax4.plot(valid_areas, valid_speedups, 'go-', label='Speed-up Ratio', linewidth=2, markersize=8)
        ax4_twin = ax4.twinx()
        ax4_twin.plot(valid_areas, valid_mem_ratios, 'mo-', label='Memory Ratio', linewidth=2, markersize=8)
        
        ax4.set_xlabel('Dataset Area (km²)')
        ax4.set_ylabel('Speed-up Ratio (Deprecated/Optimized)', color='g')
        ax4_twin.set_ylabel('Memory Ratio (Deprecated/Optimized)', color='m')
        ax4.set_title('Performance Improvement Ratios')
        ax4.grid(True, alpha=0.3)
        ax4.axhline(y=1, color='k', linestyle='--', alpha=0.5)
        ax4_twin.axhline(y=1, color='k', linestyle='--', alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig('/Users/simonvirgo/PycharmProjects/Mineye-Terranigma/performance_comparison.png', 
...
def print_summary(results: Dict):
    ...
    for i, roi_name in enumerate(results['roi_names']):
        print(f"\n{roi_name}:")
        print(f"  Area: {results['roi_areas'][i]:.2f} km²")
        print(f"  Pixels: {results['roi_pixels'][i]:,}")
        
        if results['bayseg_success'][i]:
            print(f"  BaySeg (Optimized):")
            print(f"    Time: {results['bayseg_times'][i]:.2f}s")
            print(f"    Memory: {results['bayseg_memory'][i]:.2f} MB")
        else:
            print(f"  BaySeg (Optimized): FAILED")
            
        if results['deprecated_success'][i]:
            print(f"  BaySeg Deprecated:")
            print(f"    Time: {results['deprecated_times'][i]:.2f}s")
            print(f"    Memory: {results['deprecated_memory'][i]:.2f} MB")
        else:
            print(f"  BaySeg Deprecated: FAILED")
        ...
        if results['bayseg_success'][i] and results['deprecated_success'][i]:
            ...
            if results['bayseg_memory'][i] > 0:
                memory_ratio = results['deprecated_memory'][i] / results['bayseg_memory'][i]
                if memory_ratio > 1:
                    print(f"  Memory efficiency: {memory_ratio:.2f}x better")
                else:
                    print(f"  Memory usage: {1/memory_ratio:.2f}x more")
    
    # Overall statistics
    successful_bayseg = sum(results['bayseg_success'])
    successful_deprecated = sum(results['deprecated_success'])
    total_tests = len(results['roi_names'])
    
    print(f"\nOverall Results:")
    print(f"  Total tests: {total_tests}")
    print(f"  BaySeg (Optimized) success rate: {successful_bayseg}/{total_tests}")
    print(f"  BaySeg Deprecated success rate: {successful_deprecated}/{total_tests}")
    
    if successful_bayseg > 0 and successful_deprecated > 0:
        avg_bayseg_time = np.mean([t for i, t in enumerate(results['bayseg_times']) if results['bayseg_success'][i]])
        avg_deprecated_time = np.mean([t for i, t in enumerate(results['deprecated_times']) if results['deprecated_success'][i]])
        avg_speedup = avg_deprecated_time / avg_bayseg_time if avg_bayseg_time > 0 else 0
        
        avg_bayseg_memory = np.mean([m for i, m in enumerate(results['bayseg_memory']) if results['bayseg_success'][i]])
        avg_deprecated_memory = np.mean([m for i, m in enumerate(results['deprecated_memory']) if results['deprecated_success'][i]])
        
        print(f"  Average speed improvement: {avg_speedup:.2f}x")
        print(f"  Average BaySeg memory: {avg_bayseg_memory:.2f} MB")
        print(f"  Average Deprecated memory: {avg_deprecated_memory:.2f} MB")


def main():
    """Main function to run the performance comparison."""
    
    print("BaySeg Performance Comparison Script")
    print("This script compares the performance of BaySeg vs BaySeg_Deprecated")
    print("using Tharsis segmentation data of different sizes.\n")
